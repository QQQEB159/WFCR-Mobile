...
class Character extends FlxSprite
{
	...
	public var ignoreNoteTypes:Array<String> = [];
	...
	function loadFromPsychData(json:CharacterFile)
	{
		...
		if (json.healthbar_colors != null && json.healthbar_colors.length > 2){
			...
			healthColor = FlxColor.fromRGB(healthColorArray[0], healthColorArray[1], healthColorArray[2]);
		}
 ...
 */
	...
	public function playNote(note:Note, field:PlayField) {
		...
		if (note.noAnimation || animTimer > 0.0 || voicelining || ignoreNoteTypes.contains(note.noteType.toLowerCase()))
			return;
