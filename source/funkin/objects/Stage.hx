...
class Stage extends FlxTypedGroup<FlxBasic>
{
	...
	**/ 
	public static function getTitleStages(modsOnly = false):Array<String>{
		...
		#end
		if(!Progress.getCondition("playedMatt")){
			daList.remove("phase2");
			daList.remove("sportingidklol");
		}
		if(!Progress.getCondition("playedAkiro")){
			daList.remove("akirolol");
		}
		if(!Progress.getCondition("playedTommy")){
			daList.remove("corruptedfrisbee");
		}
