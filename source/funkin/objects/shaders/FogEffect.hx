...
/**
 * Ported from https://www.shadertoy.com/view/XtfSW4
 */
class FogEffect {
	public var shader(default, null):FogShader = new FogShader();
	public var cloudDensity(default, set):Float = 0.0;
	public var noisiness(default, set):Float = 0.35;
	public var speed(default, set):Float = 0.1;
    public var cloudHeight(default, set):Float = 2.5;
	...
	public inline function update(elapsed:Float):Void {
		shader.iTime.value[0] += elapsed;
	}

	function set_cloudDensity(v:Float):Float {
		cloudDensity = v;
		shader.cloudDensity.value = [v];
		return v;
	}

	function set_noisiness(v:Float):Float {
		noisiness = v;
		shader.noisiness.value = [v];
		return v;
	}

    function set_speed(v:Float):Float {
		speed = v;
		shader.speed.value = [v];
		return v;
	}

    function set_cloudHeight(v:Float):Float {
		cloudHeight = v;
		shader.cloudHeight.value = [v];
		return v;
	}

    function set_color(v:FlxColor):FlxColor {
        color = v;
        shader.color.value = [v.red/255, v.green/255, v.blue/255];
        return v;
    }
}
...
class FogShader extends FlxShader {
	@:glFragmentSource('
        ...
        void mainImage( out vec4 fragColor, in vec2 fragCoord )
        {
            ...
            float q = max(gradient(uv + duv) * cloudDensity,0.0);
            fragColor = vec4(mix(texture(iChannel0, uv).rgb, color, q), 1.0);
        }
