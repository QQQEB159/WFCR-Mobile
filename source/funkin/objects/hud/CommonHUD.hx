...
class CommonHUD extends BaseHUD {
	...
	public function new(iP1:String, iP2:String, songName:String, stats:Stats) {
		...
		timeTxt.setFormat(Paths.font("droidbold.ttf"), 32, 0xFFFFFFFF, CENTER, FlxTextBorderStyle.OUTLINE, 0xFF000000);
	...
	function updateTimeBarType() {
		...
		if (ClientPrefs.timeBarType == 'Song Name') {
			timeTxt.text = songName.replace("-", " ").capitalize();
			timeTxt.size = 24;
			timeTxt.offset.y = -3;
		} else {
			timeTxt.text = "";
	...
	inline function updateTimeBarAlpha() {
		timeTxt.alpha = timeBar.alpha = timeBarBG.alpha = ClientPrefs.timeOpacity * alpha * tweenProg;
	}
