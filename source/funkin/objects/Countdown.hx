...
class Countdown {
	...
	@:noCompletion function get_active()
		return timer != null && timer.active;

	@:noCompletion function get_finished()
		...
		if (sprImage != null) {
			...
			try {
				if (defaultTransition) {
					if (game == null)
						parent.insert(parent.members.length - 1, sprite);
					else
						game.insert(game.members.indexOf(game.notes), sprite); // how it was layered originally
