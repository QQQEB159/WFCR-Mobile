...
class DropShadowShader extends FlxShader
{
  ...
  public var split(default, set):Float = 0;
  ...
  function set_split(val:Float):Float
  {
    split = val;
    _split.value = [val];
    return split;
  ...
  @:glFragmentSource('
      ...
      void main()
      {
        ...
        col.r = flixel_texture2D(bitmap, vec2(uv.x+_split, uv.y)).r;
			  col.b = flixel_texture2D(bitmap, vec2(uv.x+_split, uv.y)).b;

        col = col * (1.0 - _split * 0.5);
