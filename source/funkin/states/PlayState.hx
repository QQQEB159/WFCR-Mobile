...
import funkin.states.PauseSubState.SkipTimeOption;
...
import funkin.states.CorruptPause;
...
class PlayState extends MusicBeatState {
	...
	private static function get_song()
		return songPlaylist[songPlaylistIdx];
	...
	public static var storySaveString:String = "";

	public static var validStoryRun:Bool = true;
	...
	public static var playedTillEnd:Bool = false;
	...
	public var eventNotesSteps:Array<EventNote> = [];
	...
	public var rpcIcon:String = "icon";
	...
	@:noCompletion function get_beatsPerZoom()
		return zoomEveryBeat;

	@:noCompletion function set_beatsPerZoom(val:Int)
	...
	@:noCompletion public inline function get_totalPlayed()
		return stats.totalPlayed;

	@:noCompletion public inline function get_totalNotesHit()
		return stats.totalNotesHit;

	@:noCompletion public inline function get_combo()
		return stats.combo;

	@:noCompletion public inline function get_cbCombo()
		return stats.cbCombo;

	@:noCompletion public inline function get_ratingName()
		return stats.grade;

	@:noCompletion public inline function get_ratingPercent()
		return stats.ratingPercent;

	@:noCompletion public inline function get_ratingFC()
		return stats.clearType;

	@:noCompletion public inline function set_songScore(val:Int)
		return stats.score = val;

	@:noCompletion public inline function set_totalPlayed(val:Float)
		return stats.totalPlayed = val;

	@:noCompletion public inline function set_totalNotesHit(val:Float)
		return stats.totalNotesHit = val;

	@:noCompletion public inline function set_combo(val:Int)
		return stats.combo = val;

	@:noCompletion public inline function set_cbCombo(val:Int)
		return stats.cbCombo = val;

	@:noCompletion public inline function set_ratingName(val:String)
		return stats.grade = val;

	@:noCompletion public inline function set_ratingPercent(val:Float)
		return stats.ratingPercent = val;

	@:noCompletion public inline function set_ratingFC(val:String)
		return stats.clearType = val;
	...
	public var modchartShaders:Map<String, CustomShader> = new Map();
	public var modchartCameras:Map<String, LuaCamera> = new Map();
	...
	override public function create() {
		...
		FlxG.sound.cache("assets/sounds/pause/wii_sfx_1.ogg");
		curStep = FlxMath.MIN_VALUE_INT; // make sure the step is actually reset.
		Highscore.loadData();
		...
		modchartCameras.set("game", {cam: camGame, shaders: [], shaderNames: []});
		modchartCameras.set("hud", {cam: camHUD, shaders: [], shaderNames: []});
		modchartCameras.set("overlay", {cam: camOverlay, shaders: [], shaderNames: []});
		modchartCameras.set("other", {cam: camOther, shaders: [], shaderNames: []});
		modchartCameras.set("underlay", {cam: camStageUnderlay, shaders: [], shaderNames: []});
		...
		rpcIcon = charToIcon.get(dad.curCharacter) ?? 'logo';
		...
		if (hud == null) {
			...
			switch (ClientPrefs.etternaHUD) {
				case 'Advanced':
					hud = new AdvancedHUD(iP1, iP2, SONG.song, stats);
				case 'Kade':
					hud = new KadeHUD(iP1, iP2, SONG.song, stats);
				case 'Classic':
					hud = new ClassicHUD(iP1, iP2, SONG.song, stats);
				default:
					hud = new TraditionalHUD(iP1, iP2, SONG.song, stats);
			}
		}
		hud.cameras = [camHUD];
		...
		timingTxt.setFormat(Paths.font("droidbold.ttf"), 28, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE_FAST, FlxColor.BLACK);
		...
		if (gf != null)
			gf.callOnScripts("onAdded", [gf, null]); // if you can come up w/ a better name for this callback then change it lol
	...
	public function getLuaObject(tag:String, ?checkForTextsToo:Bool) {
		...
		if (checkForTextsToo == true && modchartTexts.exists(tag))
	...
	public function resetCountdown(countdown:funkin.objects.Countdown):Void {
		...
		if (countdown.introAlts != introAlts)
			countdown.introAlts = introAlts;
		if (countdown.introSnds != introSnds)
			countdown.introSnds = introSnds;
		if (countdown.introSoundsSuffix != introSoundsSuffix)
			countdown.introSoundsSuffix = introSoundsSuffix;
	...
	var shaderUpdateGroup:Array<String> = [];
	var iTime:Float = 0;

	function loadThatWierdVoiidChroncilesXml():Array<EventNote> {
		trace("trying to find that wierd voiid chronicles proprietary xml shit");
		if (!Paths.exists('content/wfcr/songs/${SONG.song.toLowerCase()}/modchart.xml')) {
			trace("did not find that wierd voiid chronicles proprietary xml shit");
			return [];
		}
		trace("found that wierd voiid chronicles proprietary xml shit");
		var returnEvents:Array<EventNote> = [];
		var modchart:Xml = Xml.parse(Paths.getContent('content/wfcr/songs/${SONG.song.toLowerCase()}/modchart.xml'));
		var access:Access = new Access(modchart.firstElement());
		for (event in access.node.Init.nodes.Event) {
			switch (event.att.type.toLowerCase().trim()) {
				case "initshader":
					CustomShader.initShader(event.att.name, event.att.shader);
				case "setcamerashader":
					CustomShader.setCameraShader(event.att.camera, event.att.name);
				case "setshaderproperty":
					modchartShaders.get(event.att.name).setFloat(event.att.property, Std.parseFloat(event.att.value));
				default:
					throw new haxe.exceptions.NotImplementedException('Event type ${event.att.type} is not supported or invalid!');
			}
		}
		for (event in access.node.Events.nodes.Event) {
			if (event.att.name.toLowerCase() == "colorswap") {
				returnEvents.push({
					event: "colorswap",
					value1: '${event.att.name},${event.att.property}',
					value2: '${Conductor.stepToSeconds(Std.parseInt(event.att.time))},${event.att.value},${event.att.startValue},${event.att.ease},${event.att.step}',
					strumTime: Std.parseInt(event.att.step)
				});
			} else {
				returnEvents.push({
					event: event.att.type,
					value1: '${event.att.name},${event.att.property}',
					value2: '${Conductor.stepToSeconds(Std.parseInt(event.att.time))},${event.att.value},${event.att.startValue},${event.att.ease},${event.att.step}',
					strumTime: Std.parseInt(event.att.step)
				});
		...
		for (event in access.node.Update.nodes.Event) {
			shaderUpdateGroup.push(event.att.name);
		}
		return returnEvents;
	}
	...
	function getSongEventNotes():Array<EventNote> {
		...
		if (eventsJSON != null)
			Song.getEventNotes(eventsJSON.events, allEvents);
	...
	private function generateSong(dataPath:String):Void {
		...
		eventNotesSteps = loadThatWierdVoiidChroncilesXml();
		...
		for (notetype in noteTypeMap.keys()) {
			...
			if (script != null)
				switch (script.scriptType) {
					...
					case PSYCH_LUA:
						specialLuaScripts.push(cast script);
				}
		...
		if (eventNotesSteps.length > 1)
			eventNotesSteps.sort(sortByTime);
	...
	public function generateNotes(noteData:Array<SwagSection>, callScripts:Bool = true, addToFields:Bool = true, ?playfields:Array<PlayField>,
			?notes:Array<Note>) {
		if (playfields == null)
		...
		for (section in noteData) {
			for (songNotes in section.sectionNotes) {
				...
				#end
				swagNote.sustainLength = songNotes[2] <= Conductor.stepCrotchet ? songNotes[2] : (susLength +
					1) * Conductor.stepCrotchet; // +1 because hold end
				...
				inline function makeSustain(susNote:Int, susPart:SustainPart) {
					var sustainNote:Note = new Note(daStrumTime + Conductor.stepCrochet * (susNote + 1), daColumn, prevNote, gottaHitNote, susPart, false,
						hudSkin);
					#if ALLOW_DEPRECATION
	...
	function updateSongDiscordPresence(?detailsText:String) {
		...
		if (timeLeft > 0.0)
			DiscordClient.changePresence(detailsText, stateText, rpcIcon, true, timeLeft);
		else
			DiscordClient.changePresence(detailsText, stateText, rpcIcon);
	}
	#else
	...
	override public function onFocusLost():Void {
		...
		if (ClientPrefs.autoPause && !isDead)
			DiscordClient.changePresence(detailsPausedText, stateText, rpcIcon);
		#end
	...
	override public function update(elapsed:Float) {
		...
		if (!inCutscene) {
			...
			camFollowPos.setPosition(FlxMath.lerp(camFollow.x + xOff, camFollowPos.x, lerpVal), FlxMath.lerp(camFollow.y + yOff, camFollowPos.y, lerpVal));
		...
		if (camZooming) {
			...
			camGame.zoom = FlxMath.lerp(defaultCamZoom #if NMV_MOD_COMPATIBILITY + defaultCamZoomAdd #end, camGame.zoom, lerpVal);
			camHUD.zoom = FlxMath.lerp(defaultHudZoom, camHUD.zoom, lerpVal);
		}
		...
		if (!endingSong) {
			...
			if (FlxG.keys.anyJustPressed(debugKeysChart)) {
				if (Progress.getCondition("storycomplete")) {
		...
		iTime += elapsed;
		for (shader in shaderUpdateGroup)
			modchartShaders.get(shader).setFloat("iTime", iTime);
		callOnScripts('onUpdatePost', [elapsed]);
		if (hudSkinScript != null)
			hudSkinScript.call("onUpdatePost", [elapsed]);
	}
	...
	function openChartEditor() {
		...
		if (FlxG.keys.pressed.SHIFT)
			ChartingState.curSec = curSection;
	...
	function doGameOver() {
		switch (callOnScripts('onGameOver')) {
			case Globals.Function_Stop:
				return false;
			case Globals.Function_Halt:
				return true;
		}
		...
		if (instaRespawn) {
			...
			DiscordClient.changePresence("Game Over - " + detailsText, stateText, rpcIcon);
			#end
		}
	...
	public function checkEventNote() {
		...
		while (eventNotesSteps.length > 0) {
			var daEvent = eventNotesSteps[0];

			if (curStep < daEvent.strumTime)
				break;

			triggerEventNote(daEvent.event, daEvent.value1, daEvent.value2, daEvent.strumTime);
			eventNotesSteps.shift();
		}
	}
		...
		switch (eventName) {
			...
			case 'Change Scroll Speed':
				...
				if (val2 == 0.0)
					...
					songSpeedTween = FlxTween.num(this.songSpeed, newValue, val2, {
						ease: FlxEase.linear,
			...
			case 'tweenShaderProperty':
				...
				var shader:CustomShader = modchartShaders.get(splitValue1[0]);
				if (!(splitValue1[0].toLowerCase() == "mirrorhud"
					&& splitValue1[1].toLowerCase() == "x"
					&& SONG.song.toLowerCase() == "deciimation"
					&& ClientPrefs.midScroll)) {
					if (shader != null) {
						modchartTweens.set(Std.string(splitValue1),
							FlxTween.num(Std.parseFloat(splitValue2[2]), Std.parseFloat(splitValue2[1]), Std.parseFloat(splitValue2[0]), {
								...
								onComplete: function(twn:FlxTween) {
									callOnLuas('onTweenCompleted', [Std.string(splitValue2)]);
									modchartTweens.remove(Std.string(splitValue2));
								},
								startDelay: 0
							}, (value) -> {
								shader.setFloat(splitValue1[1], value);
							}));
					} else {
						trace('Shader named ${splitValue1[0]} doesn\'t exist!');
					}
				}
			case 'Flash Camera':
				...
				if (ClientPrefs.flashing)
					camGame.flash(FlxColor.WHITE, time);
			case 'Black Flash':
				var time = Std.parseFloat(value1);
				...
				if (ClientPrefs.flashing)
					camGame.flash(FlxColor.BLACK, time);
		}
	...
	public static function gotoMenus() {
		...
		if (isStoryMode) {
			...
			if (validStoryRun){
				Progress.setData(true, storySaveString);
			}
			MusicBeatState.switchState(new StoryMenuState());
		} else {
			FreeplayState.comingFromPlayState = true;
			MusicBeatState.switchState(new CorruptFreeplayState());
		}
	...
	private function judge(note:Note, field:PlayField = null) {
		...
		if (ClientPrefs.showMS && (field == null || !field.autoPlayed)) {
			...
			FlxTween.tween(timingTxt, {y: timingTxt.y + 8}, 0.1, {
	...
	private function keyShit():Void {
		...
		for (column => actionBinds in buttonsArray) {
			if (gamepad.anyJustPressed(actionBinds))
				strumKeyDown(column);
			if (gamepad.anyJustReleased(actionBinds))
				strumKeyUp(column);
		}
	}
	...
	function goodNoteHit(note:Note, field:PlayField):Void {
		...
		//
		if (cpuControlled) {
			saveScore = false; // if botplay hits a note, then you lose scoring
			validStoryRun = false;
		}
		...
		callOnLuas('goodNoteHit', [
			notes.members.indexOf(note),
			Math.round(Math.abs(note.column)),
			note.noteType,
			note.isSustainNote,
			note.ID
		]);
	...
	public function createLua(path:String, ?scriptName:String, ?ignoreCreateCall:Bool):FunkinLua {
		...
		var script = FunkinLua.fromFile(path, scriptName, ignoreCreateCall, ["modName" => modName]);
	...
	public function createHScript(path:String, ?scriptName:String, ?ignoreCreateCall:Bool = false):FunkinHScript {
		var split = path.split("/");
		var modName:String = split[0] == "content" ? split[1] : 'assets';
		var script = FunkinHScript.fromFile(path, scriptName, ["modName" => modName], ignoreCreateCall != true);
	...
	inline public function callOnAllScripts(event:String, ?args:Array<Dynamic>, ignoreStops:Bool = false, ?exclusions:Array<String>,
			?scriptArray:Array<Dynamic>, ?vars:Map<String, Dynamic>):Dynamic
	...
	inline public function isSpecialScript(script:FunkinScript)
		return notetypeScripts.exists(script.scriptName)
	...
	#else
	inline public function callOnHScripts(event:String, ?args:Array<Dynamic>, ?vars:Map<String, Dynamic>, ignoreStops = false,
	...
	public function openPauseMenu() {
		...
		persistentUpdate = false;
		persistentDraw = true;
		openSubState(FlxG.keys.pressed.SHIFT ? new PauseSubState() : new CorruptPause());
	}
	...
	var charToIcon:Map<String, String> = [
		"corruptedmatt" => "phase1",
		"corruptedmatt2" => "phase2",
		"cakiro" => "akiro",
		"corruptedtommy" => "tommy"
	];
}
