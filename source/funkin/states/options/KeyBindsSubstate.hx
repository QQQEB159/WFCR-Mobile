	...
	override public function create()
	{
		...
		for(data in binds){
			...
			if (data.length > 1) {
				...
				text.setFormat(Paths.font("droidbold.ttf"), 28, 0xFFFFFFFF, FlxTextAlign.LEFT);
				text.updateHitbox();
				...
				text.cameras = [scrollableCam];
				text.setFormat(Paths.font("droidbold.ttf"), 32, 0xFFFFFFFF, FlxTextAlign.LEFT);
				group.add(text);
				daY += text.height;
			}
			idx++;
		...
		popupTitle.setFormat(Paths.font("droidbold.ttf"), 32, 0xFFFFFFFF, FlxTextAlign.CENTER);
		...
		popupText.setFormat(Paths.font("droidbold.ttf"), 32, 0xFFFFFFFF, FlxTextAlign.CENTER);
		...
		unbindText.setFormat(Paths.font("droidbold.ttf"), 32, 0xFFFFFFFF, FlxTextAlign.CENTER);
