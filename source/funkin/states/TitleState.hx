...
@:access(flixel.util.FlxSharedObject)
class TitleState extends MusicBeatState {
	...
	override public function create():Void {
		...
		blurFilter = new BlurFilter(32, 32, ClientPrefs.lowQuality ? openfl.filters.BitmapFilterQuality.LOW : openfl.filters.BitmapFilterQuality.HIGH);
		...
		var doEasterEgg:Bool = FlxG.random.bool(1);
		////
		var randomStage:Array<String> = doEasterEgg ? ["old", "wfcr"] : getRandomStage();
		if (randomStage != null) {
			trace(randomStage);
			...
			if (shitToLoad != null)
				funkin.data.Cache.loadWithList(shitToLoad);
			#end
		...
		for (member in bg.members) {
			cast (member, FlxSprite).shader = swagShader.shader;
		}

		logoBl = new TitleLogo(doEasterEgg ? "logo_og" : "logo");
		logoBl.exists = false;
...
class TitleLogo extends FlxSprite {
	...
	public function new(?X:Float, ?Y:Float, ?Name:String) {
		var titleGraphic = Paths.image(Name);
