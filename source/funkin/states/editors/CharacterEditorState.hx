...
class CharacterEditorState extends MusicBeatState {
	...
	override function create() {
		...
		for (i in 0...tipTextArray.length - 1) {
			...
			tipText.setFormat(Paths.font("droidbold.ttf"), 12, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE_FAST, FlxColor.BLACK);
	...
	var ghostCharDropDown:FlxScrollableDropDownMenu;
	...
	function updateGhostCharList() {
		...
		ghostList.insert(0, "");
		ghostCharDropDown.setData(FlxScrollableDropDownMenu.makeStrIdLabelArray(ghostList, true));
	}
	...
	function addGhostUI() {
		...
		ghostCharDropDown = new FlxScrollableDropDownMenu(15, 30, FlxScrollableDropDownMenu.makeStrIdLabelArray([''], true), function(pressed:String) {
			...
			if (charName != "") {
				...
				ghostAnimDropDown.setData(FlxScrollableDropDownMenu.makeStrIdLabelArray([''], true));
				ghostAnimTxt.text = "";
		...
		ghostAnimDropDown = new FlxScrollableDropDownMenu(15, ghostCharDropDown.y + 50, FlxScrollableDropDownMenu.makeStrIdLabelArray([''], true),
			function(pressed:String) {
				if (ghostChar == null)
					return;
	...
	function reloadGhost(charName:String) {
		...
		ghostAnimDropDown.setData(FlxScrollableDropDownMenu.makeStrIdLabelArray(animList, true));
	...
	function genBoyOffsets():Void {
		...
		if (char.animationsArray.length < 1) {
			...
			text.setFormat(Paths.font("droidbold.ttf"), 16, 0xFFFF0000, CENTER);
		...
		for (i => anim in char.animationsArray) {
			...
			text.setFormat(Paths.font("droidbold.ttf"), 16, isSelected ? 0xFF00FF00 : 0xFFFFFFFF, CENTER);
			text.setBorderStyle(FlxTextBorderStyle.OUTLINE_FAST, FlxColor.BLACK, 1);
			text.scrollFactor.set();
			text.cameras = [camHUD];
			dumbTexts.add(text);
		}
