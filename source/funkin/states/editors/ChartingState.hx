...
class ChartingState extends MusicBeatState {
	...
	var eventStuff:Array<Dynamic> = [
		...
		[
			'Game Flash',
			"Value 1: Hexadecimal Color (0xFFFFFFFF is default)\nValue 2: Duration in seconds (0.5 is default)"
		],
		[
			'Change Scroll Speed',
		...
		[
			...
			#if EASED_SVs + "\nValue 2: Tween settings\n(Duration and EaseFunc seperated by a / (ex. 1/quadOut))" #end
		],
		[
			"Mult SV",
	...
	override function create() {
		if(!Progress.getCondition("storycomplete")){
			throw "You arent supposed to be here";
		}
		instance = this;
		...
		_song.metadata = PlayState.SONG.metadata ?? (PlayState.song?.getMetadata("hard"));
		onLoadMetadata();
		...
		bpmTxt.setFormat(Paths.font("droidbold.ttf"), 18, 0xFFFFFFFF, LEFT, FlxTextBorderStyle.OUTLINE_FAST, 0xFF000000);
		bpmTxt.borderSize = 2;
		...
		/*
			...
			for (i in 0...tipTextArray.length) {
				...
				tipText.setFormat(Paths.font("droidbold.ttf"), 10, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE_FAST, FlxColor.BLACK);
		...
		zoomTxt.setFormat(Paths.font("droidbold.ttf"), 18, 0xFFFFFFFF, LEFT, FlxTextBorderStyle.OUTLINE_FAST, 0xFF000000);
		zoomTxt.borderSize = 2;
		zoomTxt.scrollFactor.set();
		add(zoomTxt);

		quantTxt = new FlxText(10, 200, 0, "Beat Snap: " + quantNames[curQuant], 16);
		quantTxt.setFormat(Paths.font("droidbold.ttf"), 18, 0xFFFFFFFF, LEFT, FlxTextBorderStyle.OUTLINE_FAST, 0xFF000000);
	...
	function addSongUI():Void {
		...
		var player1DropDown = new FlxScrollableDropDownMenu(10, daY + 45, FlxScrollableDropDownMenu.makeStrIdLabelArray(characters, true), function(character:String) {
		...
		var gfVersionDropDown = new FlxScrollableDropDownMenu(player1DropDown.x, player1DropDown.y + 40, FlxScrollableDropDownMenu.makeStrIdLabelArray(characters, true),
		...
		var player2DropDown = new FlxScrollableDropDownMenu(player1DropDown.x, gfVersionDropDown.y + 40, FlxScrollableDropDownMenu.makeStrIdLabelArray(characters, true),
			function(character:String) {
		...
		var stageDropDown = new FlxScrollableDropDownMenu(player1DropDown.x + 140, player1DropDown.y, FlxScrollableDropDownMenu.makeStrIdLabelArray(stages, true),
			function(character:String) {
		...
		var skinDropdown = new FlxScrollableDropDownMenu(stageDropDown.x, stageDropDown.y + 40, FlxScrollableDropDownMenu.makeStrIdLabelArray(skins, true),
	...
	function addSectionUI():Void {
		...
		////
		var swapSection:FlxButton = new FlxButton(10, check_notesSec.y + 50, "Swap section", function() {
			for (i in 0..._song.notes[curSec].sectionNotes.length) {
				var note:Array<Dynamic> = _song.notes[curSec].sectionNotes[i];
	...
	function addEventsUI():Void {
		...
		eventNameInput = new FlxUIInputText(eventDropDown.x + 1, eventDropDown.y + 1, 100, eventDropDown.selectedLabel, 8);
	...
	var waveformTrackDropDown:FlxScrollableDropDownMenu;
	...
	function addChartingUI() {
		...
		//
		waveformTrackDropDown = new FlxScrollableDropDownMenu(10, 100, FlxScrollableDropDownMenu.makeStrIdLabelArray(trackNamesArray, false), selectTrack);
		...
		panHitSounds = new FlxUICheckBox(xPos, startY + 90, null, null, 'Pan Hit Sounds', 100, () -> FlxG.save.data.chart_panHitSounds = panHitSounds.checked);
	...
	function loadTracks():Void {
		...
		var jsonTracks = PlayState.SONG.tracks;
	...
	override function update(elapsed:Float) {
		...
		bpmTxt.text = "Time: "
			...
			+ curSec
			+ "\nBeat: "
			+ math.CoolMath.floorDecimal(curDecBeat, 2)
			+ "\nStep: "
			+ curStep;
		...
		curRenderedNotes.forEachAlive(function(note:Note) {
			if (updateSelectedNote) {
				...
				if (curSelectedNote[0] == note.strumTime
					&& (curSelectedNote[2] == null ? columnToCheck < 0 : curSelectedNote[1] == columnToCheck)) {
					colorSine += elapsed;
					...
					if (note.column > -1) {
						...
						if (!note.ignoreNote) {
							...
							if (!note.hitsoundDisabled
								...
								&& (note.mustPress ? playSoundBf.checked : playSoundDad.checked)) {
	...
	function updateGrid():Void {
		...
		for (i in _song.notes[curSec].sectionNotes) {
			...
			if (i[3] != null && note.noteType != null && note.noteType.length > 0) {
				...
				daText.setFormat(Paths.font("droidbold.ttf"), 24, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE_FAST, FlxColor.BLACK);
				daText.xAdd = -32;
				daText.yAdd = 6;
				daText.borderSize = 1;
		...
		for (i in _song.events) {
			...
			if (t >= startThing && t < endThing) {
				...
				daText.setFormat(Paths.font("droidbold.ttf"), 12, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE_FAST, FlxColor.BLACK);
				daText.xAdd = -410;
				daText.borderSize = 1;
				if (note.eventLength > 1)
					daText.yAdd += 8;
	...
	function initNoteType(notetype:String) {
		if (notetype == '')
			return;
		if (notetypeScripts.exists(notetype))
			return;
	...
	function setupSusNote(note:Note):Null<FlxSprite> {
		...
		color.setHSB(((color.hue + note.colorSwap.hue * 360) % 360 + 360) % 360,
			...
			(color.brightness * 0.01 * (1.0 + note.colorSwap.brightness)) * 100.0, color.alphaFloat);
	...
	function loadJson(song:String):Void {
		...
		if(daJson == null){
			daJson =  Song.loadFromJson(song.toLowerCase()+"-hard", song.toLowerCase());
		}
