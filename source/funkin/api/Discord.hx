...
private final defaultRPCInfo:DiscordRPCInfo = {
	applicationId: '1200594329954177084',
	allowedImageKeys: ['icon', 'phase1', 'phase2', 'akiro', 'tommy'],

	defaultLargeImageText: 'Wii Funkin\' Corruption: Reborn',
...
class DiscordClient
{
		...
		while (true) {
			...
			if (msg==false) {
				...
				if (isActive != true) {
					...
					DiscordRpc.Shutdown();
					var eventHandler:DiscordEventHandlers =  new DiscordEventHandlers();
					...
					DiscordRpc.Initialize(curId, cpp.RawPointer.addressOf(eventHandler), true, null);
	...
	public static function changePresence(details:String, ?state:String, ?largeImageKey:String, ?hasStartTimestamp:Bool, ?endTimestamp:Float)
	{
		...
		lastPresence = new DiscordRichPresence();
