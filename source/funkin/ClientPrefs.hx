	...
	inline public static function getOptionDefinitions():Map<String, OptionData>
	{
		return [
			...
			"storageType" => {
				...
				type: Dropdown,
				value: "EXTERNAL",
				data: ["options" => ["EXTERNAL_DATA", "EXTERNAL_OBB", "EXTERNAL_MEDIA", "EXTERNAL"]]
			},
			"hitboxType" => {
...
/* 			"optimizeHolds" => {
			...
			"vSync" => {
				display: "VSync",
				desc: "When toggled, VSync will be enabled, making the game's framerate match your refresh rate.",
				type: Toggle,
				value: false,
				data: ["recommendsRestart" => false]
			},
			"lowQuality" => {
				display: "Low Quality",
