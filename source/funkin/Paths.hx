...
class Paths
{
	...
	*/
	...
	public static function getSound(key:String):Null<Sound> {
		#if sys
		var opusPath:String = key.replace(".ogg", ".opus");
		var opusPathMods:String = opusPath.replace("assets", "content/wfcr");
		if(FileSystem.exists(opusPath)){
			return Opus.toOpenFL(opusPath);
		}
		if(FileSystem.exists(opusPathMods)){
			return Opus.toOpenFL(File.getBytes(opusPathMods));
		}
