...
class FunkinLua extends FunkinScript {
	...
	override function setDefaultVars() {
		...
		//// mod manager
		addCallback("setPercent", function(modName:String, val:Float, player:Int = -1) PlayState.instance.modManager.setPercent(modName, val, player));
		...
		addCallback("setValue", function(modName:String, val:Float, player:Int = -1) PlayState.instance.modManager.setValue(modName, val, player));

		addCallback("getPercent", function(modName:String, player:Int) return PlayState.instance.modManager.getPercent(modName, player));

		addCallback("getValue", function(modName:String, player:Int) return PlayState.instance.modManager.getValue(modName, player));

		addCallback("queueSet",
			function(step:Float, modName:String, target:Float, player:Int = -1) PlayState.instance.modManager.queueSet(step, modName, target, player));

		addCallback("queueSetP",
			function(step:Float, modName:String, perc:Float, player:Int = -1) PlayState.instance.modManager.queueSetP(step, modName, perc, player));

		addCallback("queueEase",
			function(step:Float, endStep:Float, modName:String, percent:Float, style:String = 'linear', player:Int = -1,
					?startVal:Float) PlayState.instance.modManager.queueEase(step, endStep, modName, percent, style, player, startVal));

		addCallback("queueEaseP",
			function(step:Float, endStep:Float, modName:String, percent:Float, style:String = 'linear', player:Int = -1,
					?startVal:Float) PlayState.instance.modManager.queueEaseP(step, endStep, modName, percent, style, player, startVal));
		...
		addCallback("objectPlayAnim",
			function(obj:String, name:String, forced:Bool = false, ?startFrame:Int = 0) { // some psych mods use this but i cant find it any official psych code
				// weird
		...
		addCallback("setCameraShader", function(camera:String, id:String) {
			CustomShader.setCameraShader(camera, id);
		});

		addCallback("removeCameraShader", function(camStr:String, shaderName:String) {
			if (ClientPrefs.shaders == "None") {
				return;
			}
			var cam = PlayState.instance.modchartCameras.get(camStr);
			if (cam != null) {
		...
		addCallback("setShaderProperty", function(id:String, property:String, value:Dynamic) {
			if (ClientPrefs.shaders == "None") {
				return;
			}
		...
		addCallback("tweenShaderProperty",
			function(id:String, property:String, value:Float, duration:Float, ?ease:String = "linear", ?startDelay:Float = 0.0, ?onComplete:Dynamic) {
				if (ClientPrefs.shaders == "None") {
					return;
				}
				cancelTween(id + property);
				var shader:CustomShader = PlayState.instance.modchartShaders.get(id);
				if (shader != null) {
					PlayState.instance.modchartTweens.set(id + property, FlxTween.num(shader.getFloat(property), value, duration, {
						ease: getFlxEaseByString(ease),
						onComplete: function(twn:FlxTween) {
							if (onComplete != null)
								onComplete();
							PlayState.instance.callOnLuas('onTweenCompleted', [id + property]);
							PlayState.instance.modchartTweens.remove(id + property);
						},
						startDelay: startDelay
					}, (value) -> {
						shader.setFloat(property, value);
					}));
				} else {
					luaTrace('Shader named $shader doesn\'t exist!');
				}
			});

		addCallback("setShaderArray", function(obj:String, prop:String, values:Dynamic) {
			if (ClientPrefs.shaders == "None") {
				return;
			}
			#if (!flash && MODS_ALLOWED && sys)
			var shader:CustomShader = PlayState.instance.modchartShaders.get(obj);
			if (shader == null)
				return;
		...
		addCallback("getShaderPropertyFloat", function(obj:String, prop:String):Float {
			if (ClientPrefs.shaders == "None") {
				return 0;
			}
			#if (!flash && MODS_ALLOWED && sys)
			...
			return Std.parseFloat(Std.string(shader.getFloat(prop)));
			#else
			LuaUtils.luaTrace(lua, "getShaderPropertyFloat: Platform unsupported for Runtime Shaders!", false, false, FlxColor.RED);
			return 0;
			#end
		});

		addCallback("setCameraShaderBlurEffect", function(camera:String) {
			if (ClientPrefs.shaders == "None") {
				return;
			}
			var cam = PlayState.instance.modchartCameras.get(camera);
			if (cam != null) {
				cam.shaders.push(new openfl.filters.BlurFilter(0, 0,
					ClientPrefs.lowQuality ? openfl.filters.BitmapFilterQuality.LOW : openfl.filters.BitmapFilterQuality.HIGH));
				cam.cam.filters = cam.shaders;
			}
		});

		addCallback("setBlurEffectProperty", function(camera:String, property:String, value:Float) {
			if (ClientPrefs.shaders == "None") {
				return;
			}
			var cam = PlayState.instance.modchartCameras.get(camera);
			for (filter in cam.cam.filters) {
				if (filter is openfl.filters.BlurFilter) {
					Reflect.setProperty(filter, property, value);
				}
			}
		});

		addCallback("tweenBlurEffect",
			function(camera:String, property:String, value:Float, duration:Float, ?ease:String = "linear", ?startDelay:Float = 0.0, ?onComplete:Dynamic) {
				if (ClientPrefs.shaders == "None") {
					return;
				}
				var cam = PlayState.instance.modchartCameras.get(camera);
				for (filter in cam.cam.filters) {
					if (filter is openfl.filters.BlurFilter) {
						PlayState.instance.modchartTweens.set(camera + property, FlxTween.num(Reflect.getProperty(filter, property), value, duration, {
							ease: getFlxEaseByString(ease),
							onComplete: function(twn:FlxTween) {
								if (onComplete != null)
									onComplete();
								PlayState.instance.callOnLuas('onTweenCompleted', [camera + property]);
								PlayState.instance.modchartTweens.remove(camera + property);
							},
							startDelay: startDelay
						}, (value) -> {
							Reflect.setProperty(filter, property, value);
		...
		addCallback("focusWindow", function() {
			FlxG.stage.window.focus();
		});

		addCallback("bound", function(value:Float, ?Min:Float, ?Max:Float) {
			return FlxMath.bound(value, Min, Max);
		});

		addCallback("boundTo", function(value:Float, Min:Float, Max:Float):Float {
			return CoolUtil.boundTo(value, Min, Max);
		});

		addCallback("distanceBetween", function(SpriteA:String, SpriteB:String) {
			if (PlayState.instance.modchartSprites.get(SpriteA) != null && PlayState.instance.modchartSprites.get(SpriteB) != null)
				return FlxMath.distanceBetween(PlayState.instance.modchartSprites.get(SpriteA), PlayState.instance.modchartSprites.get(SpriteB));
			else
				return 0;
		});

		addCallback("distanceToMouse", function(sprite:String) {
			if (PlayState.instance.modchartSprites.get(sprite) != null)
				return FlxMath.distanceToMouse(PlayState.instance.modchartSprites.get(sprite));
			else
				return 0;
		});

		addCallback("distanceToPoint", function(sprite:String, x:Float, y:Float) {
			var point:FlxPoint = new FlxPoint(x, y);
			if (PlayState.instance.modchartSprites.get(sprite) != null)
				return FlxMath.distanceToPoint(PlayState.instance.modchartSprites.get(sprite), point);
			else
				return 0;
		});

		addCallback("dotProduct", function(ax:Float, ay:Float, bx:Float, by:Float) {
			return FlxMath.dotProduct(ax, ay, bx, by);
		});

		addCallback("equal", function(aValueA:Float, aValueB:Float, aDiff:Float = FlxMath.EPSILON) {
			return FlxMath.equal(aValueA, aValueB, aDiff);
		});

		addCallback("fastCos", function(n:Float) {
			return FlxMath.fastCos(n);
		});

		addCallback("fastSin", function(n:Float) {
			return FlxMath.fastSin(n);
		});

		addCallback("fastTan", function(n:Float) {
			return FlxMath.fastSin(n) / FlxMath.fastCos(n);
		});

		addCallback("fceil", function(n:Float) {
			return Math.fceil(n);
		});

		addCallback("ffloor", function(n:Float) {
			return Math.ffloor(n);
		});

		addCallback("fround", function(n:Float) {
			return Math.fround(n);
		});

		addCallback("isFinite", function(n:Float) {
			return Math.isFinite(n);
		});

		addCallback("isNaN", function(n:Float) {
			return Math.isNaN(n);
		});

		addCallback("getDecimals", function(n:Float) {
			return FlxMath.getDecimals(n);
		});

		addCallback("inBounds", function(value:Float, min:Null<Float>, max:Null<Float>) {
			return FlxMath.inBounds(value, min, max);
		});

		addCallback("isDistanceToMouseWithin", function(sprite:String, distance:Float, includeEqual:Bool = false) {
			if (PlayState.instance.modchartSprites.get(sprite) != null)
				return FlxMath.isDistanceToMouseWithin(PlayState.instance.modchartSprites.get(sprite), distance, includeEqual);
			else
				return false;
		});

		addCallback("isDistanceToPointWithin", function(sprite:String, x:Float, y:Float, distance:Float, includeEqual:Bool = false) {
			var point:FlxPoint = new FlxPoint(x, y);
			if (PlayState.instance.modchartSprites.get(sprite) != null)
				return FlxMath.isDistanceToPointWithin(PlayState.instance.modchartSprites.get(sprite), point, distance, includeEqual);
			else
				return false;
		});

		addCallback("isDistanceWithin", function(SpriteA:String, SpriteB:String, distance:Float, includeEqual:Bool = false) {
			if (PlayState.instance.modchartSprites.get(SpriteA) != null && PlayState.instance.modchartSprites.get(SpriteB) != null)
				return FlxMath.isDistanceWithin(PlayState.instance.modchartSprites.get(SpriteA), PlayState.instance.modchartSprites.get(SpriteB), distance,
					includeEqual);
			else
				return false;
		});

		addCallback("isEven", function(n:Float) {
			return FlxMath.isEven(n);
		});

		addCallback("isOdd", function(n:Float) {
			return FlxMath.isOdd(n);
		});

		addCallback("lerp", function(a:Float, b:Float, ratio:Float) {
			return FlxMath.lerp(a, b, ratio);
		});

		addCallback("maxAdd", function(value:Int, amount:Int, max:Int, min:Int = 1) {
			return FlxMath.maxAdd(value, amount, max, min);
		});

		addCallback("maxInt", function(a:Int, b:Int) {
			return FlxMath.maxInt(a, b);
		});

		addCallback("minInt", function(a:Int, b:Int) {
			return FlxMath.minInt(a, b);
		});

		addCallback("numericComparison", function(a:Float, b:Float) {
			return FlxMath.numericComparison(a, b);
		});

		addCallback("perlin", function(x:Float, y:Float, z:Float) {
			return perlin.perlin(x, y, z);
		});

		addCallback("pointInCoordinates", function(pointX:Float, pointY:Float, rectX:Float, rectY:Float, rectWidth:Float, rectHeight:Float) {
			return FlxMath.pointInCoordinates(pointX, pointX, rectX, rectY, rectWidth, rectHeight);
		});

		addCallback("random", function() {
			return Math.random();
		});

		addCallback("randomBool", function(chance:Float):Bool {
			return FlxG.random.bool(chance);
		});

		addCallback("randomFloat", function(min:Float, max:Float):Float {
			return FlxG.random.float(min, max);
		});

		addCallback("randomInt", function(min:Int, max:Int):Int {
			return FlxG.random.int(min, max);
		});

		addCallback("remapToRange", function(value:Float, start1:Float, stop1:Float, start2:Float, stop2:Float) {
			return FlxMath.remapToRange(value, start1, stop1, start2, stop2);
		});

		addCallback("round", function(v:Float) {
			return Math.round(v);
		});

		addCallback("roundDecimal", function(value:Float, percision:Int) {
			return FlxMath.roundDecimal(value, percision);
		});

		addCallback("sameSign", function(a:Float, b:Float) {
			return FlxMath.sameSign(a, b);
		});

		addCallback("signOf", function(n:Float) {
			return FlxMath.signOf(n);
		});

		addCallback("sinh", function(n:Float) {
			return FlxMath.sinh(n);
		});

		addCallback("vectorLength", function(dx:Float, dy:Float) {
			return FlxMath.vectorLength(dx, dy);
		});

		addCallback("wrap", function(value:Int, min:Int, max:Int) {
