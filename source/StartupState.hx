...
import funkin.states.FlashingLights;
import funkin.*;
...
class StartupState extends FlxTransitionableState {
	...
	public static function load():Void {
		...
		if (Progress.getData("seenWarning") == null) {
			nextState = FlashingLights;
		}

		for (value in Progress.conditions) {
			if (Progress.getData(value) == null) {
				Progress.setData(false, value);
			}
		}
	...
	override function update(elapsed:Float) {
		switch (step) {
			...
			case 10:
				...
				#else
				final waitTime:Float = (nextState == funkin.states.PlayState
					|| nextState == funkin.states.editors.ChartingState) ? 0.0 : Math.max(0.0, 1.6 - loadingTime);
				#end
